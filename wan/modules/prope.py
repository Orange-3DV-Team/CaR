...
#
# Modified for Wan2.2-5B integration

"""
PRoPE (Projective RoPE) Attention Module

This module implements the PRoPE attention mechanism from the paper
"Cameras as Relative Positional Encoding". It's used as the foundation
for UCPE (Unified Camera Positional Encoding).
...
class PropeDotProductAttention(torch.nn.Module):
    ...
    def _precompute_and_cache_apply_fns(
        ...
        rope_mode: str = 'rope',
        coeffs_t: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
    ):
...
def _prepare_apply_fns(
    ...
    Args:
        rope_mode: 'rope' or 'prope' uses standard split (camera + x + y).
                   'memrope' splits into (camera + x + y + t) where t uses
                   negative positions for memory tokens.
        coeffs_t: Precomputed (cos, sin) for temporal RoPE. Required for memrope mode.
    """
    ...
    if rope_mode == 'memrope':
        # memrope: camera (head_dim//2) + x + y + t (remainder)
        # d_x and d_y must be even for RoPE precomputation
        assert head_dim >= 6, \
            f"head_dim={head_dim} should be >= 6 for memrope mode"
        d_cam = head_dim // 2
        d_x = (head_dim // 6) // 2 * 2  # round down to even
        d_y = (head_dim // 6) // 2 * 2  # round down to even
        d_t = head_dim - d_cam - d_x - d_y
        assert d_t % 2 == 0, f"d_t={d_t} must be even for RoPE"

        # Precompute x/y coeffs with memrope dimensions
        if coeffs_x is None:
        ...
        assert coeffs_t is not None, "coeffs_t is required for memrope mode"

        transforms_q = [
            (partial(_apply_tiled_projmat, matrix=P_T), d_cam),
            (partial(_rope_apply_coeffs, coeffs=coeffs_x), d_x),
            (partial(_rope_apply_coeffs, coeffs=coeffs_y), d_y),
            (partial(_rope_apply_coeffs, coeffs=coeffs_t), d_t),
        ]
        transforms_kv = [
            (partial(_apply_tiled_projmat, matrix=P_inv), d_cam),
            (partial(_rope_apply_coeffs, coeffs=coeffs_x), d_x),
            (partial(_rope_apply_coeffs, coeffs=coeffs_y), d_y),
            (partial(_rope_apply_coeffs, coeffs=coeffs_t), d_t),
        ]
        transforms_o = [
            (partial(_apply_tiled_projmat, matrix=P), d_cam),
            (partial(_rope_apply_coeffs, coeffs=coeffs_x, inverse=True), d_x),
            (partial(_rope_apply_coeffs, coeffs=coeffs_y, inverse=True), d_y),
            (partial(_rope_apply_coeffs, coeffs=coeffs_t, inverse=True), d_t),
        ]
    else:
        # rope / prope: camera (head_dim//2) + x (head_dim//4) + y (head_dim//4)
        # Precompute cos/sin terms for RoPE.
        if coeffs_x is None:
            coeffs_x = _rope_precompute_coeffs(
                torch.tile(torch.arange(patches_x, device=device), (patches_y * cameras,)),
                freq_base=100.0,
...
def _apply_tiled_projmat(
    ...
    assert seqlen > cameras and seqlen % cameras == 0, \
        f"Per-camera projection failed: seqlen={seqlen}, cameras={cameras}, matrix.shape={matrix.shape}, feats.shape={feats.shape}"
